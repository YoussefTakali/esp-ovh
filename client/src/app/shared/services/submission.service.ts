...
export interface SubmissionReviewAppPreview {
  framework: string;
  language: string;
  provider: string;
  previewUrl: string;
...
export interface SubmissionAiIssue {
...
export interface SubmissionAiSuggestion {
  category: 'IMPROVEMENT' | 'OPTIMIZATION' | 'BEST_PRACTICE' | 'REFACTORING' | 'DOCUMENTATION';
  description: string;
  priority: 'LOW' | 'MEDIUM' | 'HIGH';
}

export interface SubmissionAiReviewResult {
  success: boolean;
  message?: string;
  rawResponse?: string;
  overallScore?: number;
  summary?: string;
  strengths?: string[];
  issues?: SubmissionAiIssue[];
  suggestions?: SubmissionAiSuggestion[];
...
export class SubmissionService {
  ...
  getTeacherReviewAppPreview(submissionId: string): Observable<SubmissionReviewAppPreview> {
    return this.http.get<SubmissionReviewAppPreview>(`${this.apiUrl}/${submissionId}/review-app`);
  }

  getTeacherSubmissionAiReview(submissionId: string, refresh: boolean = false): Observable<SubmissionAiReviewResult> {
    const params = new HttpParams().set('refresh', String(refresh));
    return this.http.get<SubmissionAiReviewResult>(`${this.apiUrl}/${submissionId}/ai-review`, { params });
  }

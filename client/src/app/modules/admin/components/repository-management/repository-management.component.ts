...
getMinValue(a: number, b: number): number {
  ...
  async fetchAllRepositories(): Promise<void> {
    const confirmed = await this.popupService.confirm({
      title: 'Bulk Fetch Repositories',
      message: 'This will fetch ALL repositories for ALL users with GitHub tokens. This may take 10-30 minutes depending on the number of users. Continue?',
  ...
  async bulkSubscribeWebhooks(): Promise<void> {
    const confirmed = await this.popupService.confirm({
      title: 'Bulk Subscribe Webhooks',
      message: 'Are you sure you want to subscribe to webhooks for all repositories? This may take a while.',
      confirmText: 'Subscribe All',
      danger: true
    });

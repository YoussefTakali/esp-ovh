...
export class StudentTasksComponent implements OnInit {
  ...
  selectedTaskDetails: StudentTask | null = null;
  loading = true;
  ...
  viewTaskDetails(taskId: string): void {
    ...
    this.studentService.getTaskDetails(taskId).subscribe({
      ...
      error: (error) => {
        ...
        this.loadingTaskDetails = false;
        this.snackBar.open('Failed to load task details. Please try again.', 'Close', {
          duration: 3500,
  ...
  openSubmissionDialog(task: StudentTask): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result?.submitted) {
        ...
        this.loadTasks();
        this.snackBar.open('Submission completed successfully!', 'Close', {
          duration: 3000,

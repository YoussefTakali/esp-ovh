...
import {
  SubmissionService,
  GradeSubmissionData,
  SubmissionReviewAppPreview,
  SubmissionAiReviewResult
} from '../../../../shared/services/submission.service';
...
export class TeacherSubmissionDetailModalComponent implements OnInit {
  ...
  activeSection: 'overview' | 'ai' | 'files' = 'overview';
  loading = false;
  error: string | null = null;
  reviewAppLoading = false;
  reviewAppError: string | null = null;
  reviewAppPreview: SubmissionReviewAppPreview | null = null;
  aiReviewLoading = false;
  aiReviewError: string | null = null;
  aiReviewResult: SubmissionAiReviewResult | null = null;
  ...
  loadSubmissionDetails(): void {
    ...
    this.submissionService.getSubmissionDetails(this.data.submissionId).subscribe({
      next: (details: any) => {
        ...
        this.loadSubmissionAiReview(false);
  ...
  loadSubmissionAiReview(forceRefresh: boolean = false): void {
    this.aiReviewLoading = true;
    this.aiReviewError = null;
    this.aiReviewResult = null;

    this.submissionService.getTeacherSubmissionAiReview(this.data.submissionId, forceRefresh).subscribe({
      next: (result) => {
        this.aiReviewResult = result;
        this.aiReviewLoading = false;

        if (!result.success) {
          this.aiReviewError = result.message || 'AI review failed for this submission.';
        }
      },
      error: (error: unknown) => {
        console.error('Error loading submission AI review:', error);
        this.aiReviewLoading = false;
        this.aiReviewError = 'Failed to generate AI commit review.';
      }
    });
  }

  setActiveSection(section: 'overview' | 'ai' | 'files'): void {
  ...
  cancelGrading(): void {
    ...
    if (this.submissionDetails?.isGraded === true) {
  ...
  getSuggestedGradeForCurrentScale(): number | null {
    if (this.aiReviewResult?.suggestedGrade === undefined || this.aiReviewResult.suggestedGrade === null) {
      return null;
    }

    const currentMax = this.gradeForm.maxGrade || this.submissionDetails?.maxGrade || 20;
    return Number(((this.aiReviewResult.suggestedGrade / 100) * currentMax).toFixed(2));
  }

  applySuggestedGrade(): void {
    const scaledSuggestion = this.getSuggestedGradeForCurrentScale();
    if (scaledSuggestion === null) {
      this.snackBar.open('No AI suggested grade available', 'Close', { duration: 2500 });
      return;
    }

    this.gradeForm.grade = scaledSuggestion;
    this.showGradeForm = true;
    this.snackBar.open('AI suggested grade applied to grading form', 'Close', { duration: 2500 });
  }
  ...
  openReviewApp(): void {
    const placeholderTab = window.open('', '_blank', 'noopener,noreferrer');
    if (placeholderTab) {
      placeholderTab.document.title = 'Preparing review app...';
      placeholderTab.document.body.innerHTML = '<p style="font-family:Arial,sans-serif;padding:20px;">Preparing app preview...</p>';
    }

    this.reviewAppLoading = true;
    this.reviewAppError = null;

    this.submissionService.getTeacherReviewAppPreview(this.data.submissionId).subscribe({
      next: (preview) => {
        this.reviewAppPreview = preview;
        this.reviewAppLoading = false;

        if (placeholderTab) {
          placeholderTab.location.href = preview.previewUrl;
        } else {
          window.open(preview.previewUrl, '_blank', 'noopener,noreferrer');
        }

        this.snackBar.open(`Preview ready: ${preview.framework} on ${preview.provider}`, 'Close', { duration: 3500 });
      },
      error: (error: unknown) => {
        console.error('Error generating review app preview:', error);
        this.reviewAppLoading = false;
        this.reviewAppError = 'Unable to generate an app preview link for this submission.';
        if (placeholderTab) {
          placeholderTab.close();
        }
        this.snackBar.open('Failed to generate app preview link', 'Close', { duration: 3500 });
      }
    });
  }

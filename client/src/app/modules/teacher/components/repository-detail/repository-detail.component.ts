...
export class RepositoryDetailComponent implements OnInit, OnDestroy {
  ...
  async removeCollaborator(collaborator: any): Promise<void> {
    ...
    const confirmed = await this.popupService.confirm({
      title: collaborator.pending ? 'Cancel Invitation' : 'Remove Collaborator',
      message: `Are you sure you want to ${actionText} ${identifier}?`,
      confirmText: collaborator.pending ? 'Cancel Invitation' : 'Remove',
      danger: true
    });

    if (!confirmed || !this.repository) {
      return;
    }
  ...
  async confirmDelete(): Promise<void> {
    if (this.deleteInput === this.repoName && this.repository) {
      const confirmed = await this.popupService.confirm({
        ...
        message: 'This will permanently delete the repository. Are you absolutely sure?',
